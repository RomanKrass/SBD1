...
Description: We would like to forecast unemployment in Switzerland. Collect the unemployment data for the last
years (at least 10 years) broken down to Cantons. Your data could also contains additional information like
size (population, area) and any other information you think could be helpful in predicting unemployment.
Create a tsibble object from your data. Use write_rds() to save your data. Give a single plot that gives an
overview of the seasonal and trend strength of unemployment by cantons.

## Data Sources:

* Unemployment per canton: https://www.bfs.admin.ch/bfs/de/home/statistiken/arbeit-erwerb/erwerbslosigkeit-unterbeschaeftigung/registrierte-arbeitslose-seco.html
...
## Data Import:
```{r, echo=FALSE, warning=FALSE}
#TODO: remove Year 2023 for one of the datasets
# TODO: remove imports here e.g. move up to top of document
library(readxl)
library(tidyverse)
library(tsibble)
library(readr)
...
unemployment <- read_xlsx("./datasets/unemployment_numbers_switzerland.xlsx")

population <- read_xlsx("./datasets/population_numbers_switzerland.xlsx")
...
# TODO Plot the seasonal and trend strength of unemployment by cantons
# Convert the tsibble to a ts object with a specified frequency
combined_ts <- combined_tsibble %>%
  group_by(Kanton) %>%
  do(as.ts(., value = "Unemployment"))

decomposed <- combined_tsibble %>%
  model(STL(Unemployment ~ season(window = "periodic") + trend(window = 12))) %>%
  components()

print(autoplot(decomposed, features = c("seasonal", "trend")))

p <- autoplot(decomposed, features = c("seasonal", "trend"))
ggsave("plot.png", p)
```